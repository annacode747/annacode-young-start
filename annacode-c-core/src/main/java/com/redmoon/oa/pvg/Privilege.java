...
/*        if (License.TYPE_BIZ.equals(type) || License.TYPE_OEM.equals(type) || License.TYPE_SRC.equals(type)){
    ...
    public boolean setRolePriv(HttpServletRequest request) throws ErrMsgException {
        ...
        try {
            ...
            // 操作日志
            LogUtil.log(new Privilege().getUser(request), request.getRemoteAddr(), LogDb.TYPE_PRIVILEGE, LogUtil.format(request, "action_role_grant", new Object[]{rd.getDesc(), desc}));
    ...
    public boolean setGroupPriv(HttpServletRequest request) throws
        ...
        try {
            ...
            if (len > 0) {
                ...
                LogUtil.log(new Privilege().getUser(request), StrUtil.getIp(request), LogDb.TYPE_PRIVILEGE, LogUtil.format(request, "action_group_grant", new Object[]{ug.getDesc(), desc}));
            }
        } catch (SQLException e) {
            if (len > 0) {
                conn.rollback();
