package cn.annacode.org.common.tool.jieba;
...
public class WordDictionary {
    ...
    private static String MAIN_DICT = "/dict.txt";

    static {
        try {
            System.out.println(Sys.GetPathParent()+Sys.jieba);
            MAIN_DICT = Sys.GetPathParent()+Sys.jieba+"/dict.txt";
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    private WordDictionary() throws FileNotFoundException {
        this.loadDict();
    }


    public static WordDictionary getInstance() throws FileNotFoundException {
        if (singleton == null) {
    ...
    public void loadDict() throws FileNotFoundException {
        _dict = new DictSegment((char) 0);
        System.out.println("MAIN_DICT:\t"+MAIN_DICT);
        InputStream is = new FileInputStream(MAIN_DICT);
        try {
