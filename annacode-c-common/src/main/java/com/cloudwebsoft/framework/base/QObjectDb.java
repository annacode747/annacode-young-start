...
public abstract class QObjectDb implements java.io.Serializable {
	 ...
	 * 用于从表单创建对象
	 *
	 * @param paramChecker ParamChecker
	 * @return boolean
	 ...
	 * 刷新列表
	 *
	 * @param groupName String 通常以对象的分类，如圈子的类别作为groupName
	 */
					...
					if (primaryKey.getType() == PrimaryKey.TYPE_INT) {
						...
						while (ir.hasNext()) {
							...
							if (ku.getType() == PrimaryKey.TYPE_INT) {
								...
								ku.setValue(new Date(rs.getTimestamp(ku.getOrders() + 1).getTime()));
							} else {
