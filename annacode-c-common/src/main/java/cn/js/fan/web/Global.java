    ...
    public boolean initOnStart() {
        ...
        try {
            ...
            if (!Global.getInstance().isDebug()) {
                ...
                PropertiesUtil pucache = new PropertiesUtil(URLDecoder.decode(cfgURL.getFile()));
                pucache.setValue("jcs.auxiliary.DC.attributes.DiskPath", cloudHome + "CacheTemp");
                pucache.saveFile(URLDecoder.decode(cfgURL.getFile()));
            }

            if (!Global.isCluster()) {
                // XMLConfig cfg = new XMLConfig("../reportConfig.xml",false,"utf-8"); // ../reportConfig.xml路径也有效
                XMLConfig reportCfg = new XMLConfig(cloudHome + "WEB-INF" + File.separator + "reportConfig.xml", true, "utf-8");
                try {
